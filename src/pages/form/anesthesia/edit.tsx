...
const EditAnesthesia = () => {
  ...
  const [editAnesthesia, { data, error, isLoading, isSuccess, isError }] =
    useEditAnesthesiaMutation();
  const getPatients = useGetPatientsQuery({ skip: 0, limit: 100 });
  const getAnesthesia = useGetAnesthesiaQuery({ id: anesthesia });
  ...
  useEffect(() => {
    if (isSuccess) {
      setMessage(data?.msg);
      if (data?.msg == "anesthesia updated") navigate("/form/anesthesia");
    }
    ...
    if (getAnesthesia.isSuccess) {
      ...
      setSideEffect(a.sideEffect);
      setAnesthesist(a.anesthesist);
      setPatientQuestion(a.patientQuestion);
      setOperationDetails(a.operationDetails);
      setAuthorizingPerson(a.authorizingPerson);
    }
  }, [
    isError,
    isSuccess,
