...
const EditAdmission = () => {
  ...
  const [editAdmission, { data, error, isLoading, isSuccess, isError }] =
    useEditAdmissionMutation();
  const getAdmission = useGetAdmissionQuery({ id: admission });
  ...
  useEffect(() => {
    if (isSuccess) {
      setMessage(data?.msg);
      if (data?.msg == "admission updated") navigate("/form/admission");
    }
    ...
    if (getAdmission.isSuccess) {
      const a = getAdmission.data?.admission;
      setHasFled(a.hasFled);
      setPatient(a.patient);
      setIsImproved(a.isImproved);
      setReferredTo(a.referredTo);
      ...
      setModeOfAdmission(a.modeOfAdmission);
      setClinicalSummary(a.clinicalSummary);
      setTransferredFrom(a.transferredFrom);
      setWasAutopsyRequested(a.wasAutopsyRequested);
      setInvestigationSummary(a.investigationSummary);
    }
  }, [
    isError,
    isSuccess,
